...
public class EmailSendService {
    ...
    private void init() {
        ...
        logger.info("邮件发送服务session初始化完成");
    }

    public void sendEmail(Email email) throws MessagingException {
        logger.info("发送邮件,目标 : {}", email.getAddressArrStr());
        check(email);
        try {
            MimeMessage message = createMimeMessage(email);
            Transport.send(message, sender.getEmailAccount(), sender.getEmailPwd());
            logger.info("邮件发送成功,目标 : {}", email.getAddressArrStr());

        } catch (SendFailedException e) {
            logger.warn("邮件目标地址无效, message : {}", e.getMessage());
            handleSendFailedException(e, email);

        } catch (MessagingException e) {
            logger.error("网络连接失败, message : {}, 目标 : {}", e.getMessage(), email.getAddressArrStr());
            throw e;

        } catch (Exception e) {
            logger.error("邮件发送失败, message : {}, 目标 : {}", e.getMessage(), email.getAddressArrStr());
    ...
    private void handleSendFailedException(SendFailedException e, Email email) {
        ...
        if (logger.isWarnEnabled()) {
            ...
            logger.warn("错误的邮箱地址 : {}", joinAddressString(invalidAddresses));

            logger.warn("正确已发送的邮箱地址 : {}", joinAddressString(validSentAddresses));

            logger.warn("正确未发送的邮箱地址 : {}", joinAddressString(validUnsentAddresses));
        }

        if (validUnsentAddresses.length == 0) {
            logger.warn("没有正确未发送的邮箱地址，因而忽略");
            return;
        }
        ...
        try {
            ...
            logger.error("邮件发送异常，经过处理后仍无法正常发送, message : {}", e1.getMessage());
